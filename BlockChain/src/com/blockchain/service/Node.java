...
public class Node {
	...
	public static void main(String[] args) {
		...
		try {
			...
			genesisBlock.setTimeStamp(TimeUtil.getFullFormatStandardTime());
			genesisBlock.setData("创世区块");
			genesisBlock.setPreHash("");
			genesisBlock.setMyHash(Common.calculateHash(genesisBlock));
			blockChain.add(genesisBlock);

			// 建立TCP监听新块的产生(块的发现)
			serverSocket = new ServerSocket(9000);

			System.out.println("*** Node is started,waiting for others ***");
			// 监听对等网络中的结点
			while (isRunning) {
